...
OUT = os.path.join(ROOT, "outputs", "BAH2026_Problem07_Report.pdf")

# --- load real metrics -------------------------------------------------------
with open(os.path.join(ROOT, "outputs", "train_metrics.json")) as f:
    M = json.load(f)
acc = M["report"]["accuracy"]
labels = M["labels"]
cm = M["confusion"]
rep = M["report"]
...
story.append(Paragraph(
    ...
    % (M["n_train"], M["n_test"], acc * 100), body))
...
for i, lab in enumerate(labels):
    cm_data.append([lab] + [str(v) for v in cm[i]])
